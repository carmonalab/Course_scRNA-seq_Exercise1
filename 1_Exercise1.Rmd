...
-   Ready-made, well documented, extensively tested

-   Speeds up data analysis

[Points to consider]{.underline}

-   [**Which version was used?**]{.underline} A piece of code might run with one version of a package but not with an older or newer version. [**Different versions of a package might break some parts of your code or produce different results!**]{.underline} How can we make sure that our code can be reproduced by us and others now and in the future? Solution in R: use [**RENV**]{https://rstudio.github.io/renv/articles/renv.html}
...
If not previously installed, run the following
```{r, eval=F}
source("install_packages_minimal.R") 
```
...
```{r}
# Define file paths for input data and create directories to export resutls and figures
root <- getwd()
...
Note: this Peripheral Blood Mononuclear Cells (PBMC) dataset from 10X is a very "clean" dataset. In exercise 2 of this course you will have a look at the more complex breast cancer scRNA-seq data that you previously explored in the previous module of the course.
...
```
```{r}
FeaturePlot(pbmc, reduction = "pca", features=c("CD79A","LYZ","GZMB","PPBP"))
```
...
```
EXERCISE: Explore different levels of clustering granularity by changing the parameter "resolution" and then try:
```{r, eval=F}
head(pbmc@meta.data$RNA_snn_res.0.5) # the cluster IDs are store in a new metadata column. It's name depends on the configuration used, for instance if resolution of 0.5 was used, it's named "RNA_snn_res.0.5" 
...
FeaturePlot(pbmc, features = c("MS4A1","CD79A","GNLY","NKG7","CD3E","CD2","CD14","FCER1A", "FCGR3A", "LYZ", "PPBP"))
```
...
Take a snapshot of currently loaded packages, for future reproducibility
```{r, eval=F}
renv::snapshot() # this will take a snapshot of your current environment (in the file renv.lock). If you wish, you can then reproduce this environment using the call renv::restore()
```
...
VlnPlot(pbmc, features = c("MS4A1","CD79A","GNLY","KLRD1","CD3E","CD3D","CD14","FCGR3A","PPBP","CD1C"))

FeaturePlot(pbmc, features = c("MS4A1","CD79A","GNLY","KLRD1","CD3E","CD3D","CD14","FCGR3A","PPBP","CD1C"))

FeaturePlot(pbmc, reduction = "pca", features = c("MS4A1","CD79A","GNLY","KLRD1","CD3E","CD3D","CD14","FCGR3A","PPBP","CD1C"))

```

```{r}
new.cluster.ids <- c("CD3E", "CD3E", "CD14", "MS4A1", "CD3E", "CD16", "KLRD1", "CD1C", "Plat")
names(new.cluster.ids) <- levels(pbmc$RNA_snn_res.0.5)
pbmc <- RenameIdents(pbmc, new.cluster.ids)
DimPlot(pbmc, reduction = "umap", label = TRUE, pt.size = 0.5) + NoLegend()
```
