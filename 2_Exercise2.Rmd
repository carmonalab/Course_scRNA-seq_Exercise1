...
## Download the data (If not done before)


```{r}
# If it fails, download and unzip manually
...
head(colnames(brca.data)) # column names contain sample ID and cell barcode separated by "_"
```


```{r, message=FALSE, warning=F}
# Create Seurat object containin count data matrix
obj <- CreateSeuratObject(counts = brca.data, project = "brca",
                          min.cells = 3, min.features = 200, names.delim="_") 
head(obj@meta.data, 5)
# Seurat created a meta data column "orig.ident" containing the sample ID
table(obj@meta.data$orig.ident)
```
How many cells are in each sample?

```{r, message=FALSE, warning=F}
# For the sake of calculation time and RAM memory constraints in some laptops, we only keep up to 500 cells per sample (this is called downsampling)
...
In the previous exercise you annotated clusters of cells manually, finding cell type marker genes differentially expressed between clusters. 
As this is time-consuming and subjective, many tools have been developed to automate this process, including SingleR, CellTypist, Garnett and scGate.
...
scGate will output the predicted cell type of each cell in a new meta data column named "scGate_multi".
...
Automated approaches for cell type classification have an advantage over manual annotation in terms of consistency of results and scalability. In particular, marker-based approaches, such as scGate used here, have the advantage that are transparent and flexible, but depends on well defined marker genes. What if biologically relevant cell states are defined by subtle gradients of gene expression levels? At some point, you might run out of markers that are distinctly positive/negative. In this case, alternative approaches such as reference mapping (e.g. Seurat [label transfer](https://satijalab.org/seurat/articles/integration_mapping.html) and [Projectils](https://github.com/carmonalab/ProjecTILs)) might be preferable.

We have also seen that samples might present batch effects, which you might want to remove. We used silhouette scoring as a metric to quantify how mixed are cells from different batches to evaluate the batch effect before and after correction. Beware that batch effects correction might remove important biological variability in addition to technical, unwanted variation.
